...
@interface GovernmentViewController ()<UIScrollViewDelegate,UITextFieldDelegate>
{
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"园区企业";
...
- (void)initializeDataSource {
    ...
    WeakObj(self)
    NSString *url = [NSString stringWithFormat:@"%@api/Enterprise/GetTopEnterpriseTypeList",BASEURL];
    [NetManager getWithURL:url params:nil animation:YES success:^(id obj) {
        if ([obj isKindOfClass:[NSArray class]]) {
            for (NSDictionary *d in obj) {
                GoverModel *model = [[GoverModel alloc] initWithDictionary:d];
...
- (void)initializeUserInterface {
    ...
    searchTF.placeholder = SearchTFPlaceholderTextEnterpriseList;
    ...
    for (int i=0; i<self.dataArray.count; i++) {
        ...
        GoverModel *model = self.dataArray[i];
        if (i == selectedIndx) {
            _topEnterpriseTypeID = model.EnterpriseTypeID ;
            _currentType = model.EnterpriseTypeName ;
            [self getDetail];
        }
        ...
        [button setTitle:model.EnterpriseTypeName forState:UIControlStateNormal];
...
- (void)showDetailInfo:(UIButton *)sender {
    ...
    GoverModel *model = _dataArray[sender.tag - 300];
    NSURL *URLLLLL = [NSURL URLWithString:model.EnterpriseTypeIcon];
...
- (void)getDetail {
    ...
    NSString *url = [NSString stringWithFormat:@"%@api/Enterprise/GetSecondEnterpriseTypeList",BASEURL];
    NSDictionary *param = @{@"topEnterpriseTypeID":_topEnterpriseTypeID};
    WeakObj(self)
...
- (void)detailView {
    ...
    for (int i=0; i<self.deatilArray.count; i++) {
        GoverModel *model = self.deatilArray[i];
        NSString *companyCount = model.EnterpriseCount;
        companyCount = [NSString stringWithFormat:@"%@家",companyCount];
        NSString *companyType = model.EnterpriseTypeName;
...
- (void)labelGesture:(UITapGestureRecognizer *)tap {
    ...
    GoverModel *model = self.deatilArray[indx];
    _secondEnterpriseTypeID = model.EnterpriseTypeID;
    [self pushToEnterprise];
}

- (void)pushToEnterprise {
    
    EnterpriseListViewController *vc = [[EnterpriseListViewController alloc] init];
    vc.topEnterpriseTypeID = _topEnterpriseTypeID;
    vc.secondEnterpriseTypeID = _secondEnterpriseTypeID;
